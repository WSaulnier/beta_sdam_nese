---
title: "Beta Streamflow Duration Assessment for the Great Plains"
subtitle: "SDAM Version 1.0"
revision: "SDAM revision date: July, 2022" 
...
Great Plains Region: `r params$gp_region`
...
Number of EPT families identified from the assessment reach: `r if (!is.null(params$aqua_inv)) {params$aqua_inv}`
...
Number of hydrophytic plant species identified from the assessment reach without odd distribution: `r if (!is.null(params$hydrophytes)) {params$hydrophytes}`

Notes on hydrophytes:

```{r}
cat(params$notes_hydrophytes)
```
...
Sinuosity score: `r if (!is.null(params$sinuosity)){params$sinuosity}`
...
Floodplain and channel dimensions score: `r if (!is.null(params$channel_dim)){params$channel_dim}`
...
Notes about channel dimensions:

```{r}
cat(params$notes_channel)
```


Floodplain and channel dimensions photos and descriptions:
```{r, out.width='50%', fig.align='center'}
